...
while getopts "d:c:x:l:t:v:" opt; do
	case "$opt" in
	d)
		INPUT="$OPTARG"
		mode="DUMMY"
...
main() {
	...
	if [ ! -d "$EN_HOLD_DIR" ]; then
		printf "${YELLOW}Warning! Directory doesn't existed [EN_HOLD_DIR][$EN_HOLD_DIR]${NC}\n"
		validate=1
	fi
	if [ ! -d "$ES_HOLD_DIR" ]; then
		printf "${YELLOW}Warning! Directory doesn't existed [ES_HOLD_DIR][$ES_HOLD_DIR]${NC}\n"
		validate=1
	fi
	if [ ! -d "$FR_HOLD_DIR" ]; then
		printf "${YELLOW}Warning! Directory doesn't existed [FR_HOLD_DIR][$FR_HOLD_DIR]${NC}\n"
		validate=1
	fi
