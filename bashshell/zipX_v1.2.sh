...
if [ ! -d "$FAIL_PATH" ]; then
  printf "${YELLOW}Warning! Directory doesn't existed [FAIL_PATH][$FAIL_PATH]${NC}\n"
  validate=1
fi
if [ ! -d "$OVER_THRESHOLD_PATH" ]; then
  printf "${YELLOW}Warning! Directory doesn't existed [OVER_THRESHOLD_PATH][$OVER_THRESHOLD_PATH]${NC}\n"
  validate=1
fi
if [ ! -d "$UNDER_THRESHOLD_PATH" ]; then
  printf "${YELLOW}Warning! Directory doesn't existed [UNDER_THRESHOLD_PATH][$UNDER_THRESHOLD_PATH]${NC}\n"
  validate=1
fi
if [ ! -d "$ARCHIVE_PATH" ]; then
  printf "${YELLOW}Warning! Directory doesn't existed [ARCHIVE_PATH][$ARCHIVE_PATH]${NC}\n"
  validate=1
fi
if [ ! -d "$PARK" ]; then
  printf "${YELLOW}Warning! Directory doesn't existed [PARK][$PARK]${NC}\n"
  validate=1
fi
if [ ! -d "$LOG_PATH" ]; then
