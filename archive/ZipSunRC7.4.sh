...
process_zip_file(){
  ...
  if [ ${#sub_folder[@]} -gt 0 ];then
    # unzip
    rm -rf "$TMP_DIR"
    tmp_random=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 8 | head -n 1)
    tmp_random="$TMP_DIR/$tmp_random"
    mkdir -p "$$tmp_random"
    # Get root zip directory name
    root_dir_name=$(basename $(zipinfo -1 "$file_path" | head -n 1))
    for d in "${sub_folder[@]}";do
      ...
      for i in "${!arrFiles[@]}";do 
        ...
        unzip -jo -qq  "$file_path" "${arrFiles[$i]}" -d "$tmp_random"
      done

      echo
      echo "Start processing video ..."
      files=$(ls -S "$tmp_random")
      while read file; do
        count=$(($count +1))
        file="$tmp_random/$file"
